...
//R00k: TODO,make a cvar: sv_gameplay_airmove_fix
...
// read current angles
	if (sv.protocol == PROTOCOL_FITZQUAKE)// johnfitz -- 16-bit angles for PROTOCOL_FITZQUAKE
	{
		for (i=0 ; i<3 ; i++)
	...
	if (pq_fullpitch.value)
		angle[PITCH] = bound (-90, angle[PITCH], 90);
	else
		angle[PITCH] = bound (-70, angle[PITCH], 80);
