...
void SV_Init (void)
{
	...
	//R00k create a default map to load
	Cvar_RegisterVariable (&sv_defaultmap);
	...
	Cvar_RegisterVariable (&sv_freezenonclients);//DarkPlaces
	Cvar_RegisterVariable (&sv_disable_noclip_touch_triggers);
...
void SV_ConnectClient (int clientnum)
{
	...
	Con_DPrintf (1,"Client %s connected\n", client->netconnection->address);
...
=============
*/
void SV_WriteEntitiesToClient (edict_t	*client, sizebuf_t *msg)
{
	int		e, i;
	int		bits;
	byte	*pvs;
...
// send over all entities (except the client) that touch the pvs
	...
	for (e = 1 ; e < sv.num_edicts ; e++, ent = NEXT_EDICT(ent))
	{
		...
		if (ent != client)	// player is always sent
		{
			...
			// MH: link to PVS leafs - deferred to here so that we can compare leafs that are touched to the PVS.
			// this is less optimal on one hand as it now needs to be done separately for each client, rather than once
			// only (covering all clients), but more optimal on the other hand as it only needs to hit one leaf and will
			// start dropping out of the recursion as soon as it does so; it should be more optimal overall.
			ent->touchleaf = false;
			...
			if (sv_cullentities.value > 0)
			{
				if (SV_InvisibleToClient(client, ent))
				{
					continue;		
				}
			}
		...
		{
			if (alpha < 1 && alpha > 0) bits |= U_ALPHA;
	...
	//
		...
		if ((bits & U_TRANS)&&(sv.protocol == PROTOCOL_NETQUAKE))
		{
			MSG_WriteFloat (msg, 2);
			MSG_WriteFloat (msg, trans);
...
// build individual updates
	for (i=0, host_client = svs.clients ; i<svs.maxclients ; i++, host_client++)
	{
		...
		if (host_client->message.overflowed)
		{
			...
			SZ_Clear (&host_client->message);		//R00k Added.
			host_client->last_message = realtime;	//R00k Added.
			host_client->sendsignon = false;		//R00k Added.
			continue;
		}
...
// set up the new server
	...
	// level's data which is no longer valid
	cls.signon = 0;
...
// leave slots at start for clients only
	...
	//R00k if map isnt found then load the sv_defaultmap instead
	if (!sv.worldmodel)
	{
		if (sv_defaultmap.string)//R00k if sv_defaultmap is null then return with error.
		{
			strcpy (sv.name, sv_defaultmap.string);
			sprintf (sv.modelname,"maps/%s.bsp", sv_defaultmap.string);
			Cvar_Set ("mapname", sv_defaultmap.string);	
			sv.worldmodel = Mod_ForName (sv.modelname, false);
		}
		else
		{
