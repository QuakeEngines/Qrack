...
void Mod_ClearAll (void)
{
	...
	for (i=0 , mod=mod_known ; i<mod_numknown ; i++, mod++) 
	{
		...
		if (mod->type == mod_sprite) //FIXED FOR CACHE_ALLOC ERRORS:
			mod->cache.data = NULL;
	}
}
...
// load the frames
	...
	for (i=0 ; i<numframes ; i++)
	{
		...
		{
			pframetype = (dspriteframetype_t *)Mod_LoadSpriteFrame (pframetype + 1,
							&psprite->frames[i].frameptr);
		}
		else
		{
			pframetype = (dspriteframetype_t *)Mod_LoadSpriteGroup (pframetype + 1,
							&psprite->frames[i].frameptr);
		}
	}
