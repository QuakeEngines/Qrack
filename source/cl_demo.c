...
void CL_FreeMove (void)
{
	...
	wishvel[2] += cl.cmd.upmove;
	
	cl.crouch = 0;
//R00k_FIXME: vector copy cl origin to demo origin, then move via wishdir this will fix the up down movement when freelook/freemoving
	//R00k allow us to move normally , Removed host_frametime as this is fps dependent
	cls.demoentity.origin[0] += wishvel[0] * 0.005;//host_frametime; 
	cls.demoentity.origin[1] += wishvel[1] * 0.005;//host_frametime;
	cls.demoentity.origin[2] += wishvel[2] * 0.005;//host_frametime;	
}
...
void PushFrameposEntry (long fbaz)
{
	...
	newf = (framepos_t *) malloc (sizeof (framepos_t));
	newf->baz = fbaz;
...
int CL_GetMessage (void)
{
	...
	if (cls.demoplayback)
	{
		...
		//get mouse movement for freelook		
		if ((freelooking.value) || (freemoving.value))
		{
			IN_MouseMove(&cmd);			
			for (i=0 ; i<3 ; i++)
			{
				cl.mviewangles[1][i] = cl.mviewangles[0][i];
		...
		{
			Con_Print("Demo Error: message size > 8000 bytes.\n Can not play demo.\n");
			CL_StopPlayback ();
			return 0;
		}
		...
		if (cl_demorewind.value)
		{
			if (dem_framepos)				//RMQe joequake bug - dem_framepos can be NULL coming in here...
			{
...
void CL_Record_f (void)
{
	...
	if (c == 1 || c == 2)
	{
		if (c == 1)
		{
			...
			Q_snprintfz (easyname, sizeof(easyname), "%s-%s", sv_mapname.string, str);
		}
		else if (c == 2)
		{
	...
	if (easyname[0])
		Q_snprintfz (name, sizeof(name), "%s/demos/%s", com_gamedir, easyname);
	else
		Q_snprintfz (name, sizeof(name), "%s/demos/%s", com_gamedir, Cmd_Argv(1));
...
// open the demo file
	...
	if (c < 3 && cls.state == ca_connected)
	{
		...
		// send stats
		//R00k: changed to cl.stats[ so stats for coop games can be saved too!
...
void StartPlayingOpenedDemo (void)
{
	...
	freelooking.value = 0;//R00k - for freelook
	freemoving.value = 0;//R00k - for freemove
}
