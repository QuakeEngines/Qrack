...
cvar_t	show_status_ip	= {"show_status_ip","1"};
...
void Host_Status_f (void)
{
	...
	for (j=0, client = svs.clients ; j<svs.maxclients ; j++, client++)
	{
		...
		if (show_status_ip.value) //R00k: hide static ips from streams
		{
			if (cmd_source != src_command && sscanf(client->netconnection->address, "%d.%d.%d", &a, &b, &c))  //RocketGuy
				print ("   %d.%d.%d.xxx\n", a, b, c);  // engine side ip masking from RocketGuy
		}
	}
}
...
void Host_Changelevel_f (void)
{
	...
	if (Cmd_Argc() != 2)
	{
		//this is called in quakeC, no one really reads this message.
		//Con_Printf ("changelevel <levelname> : continue game on a new level\n");
		Con_Printf ("Loading Random Level...\n");
		Host_LoadRandomLevel();
		return;
	}
...
{		
	...
	if ((cl_autodemo.value == 1) && (cls.demorecording))
		CL_Stop_f();
...
// remove quotes if present
	...
	if (!fromServer)
	{
		...
		if ((Sys_DoubleTime()) - host_client->netconnection->connecttime < 5)		
			return;
...
void Host_Spawn_f (void)
{
	...
	extern	void CL_UpdateDemoStat (int stat);
...
// run the entrance script
	...
	{	// loaded games are fully inited allready
		...
		if ((f = ED_FindFunction("RestoreGame")))
		{
			...
			{
				Con_DPrintf (1,"Calling RestoreGame\n");
				pr_global_struct->time = sv.time;
				pr_global_struct->self = EDICT_TO_PROG(sv_player);
...
// with a permanent head tilt
	...
	for (i=0 ; i < 2 ; i++)
		MSG_WriteAngle (&host_client->message, sendangle[i]);//MH: updated for correct angle.
...
//R00k: Look for a particular edict based in it's classname.
edict_t	*FindClassEdict (char *name)
{
	int		i;
	edict_t	*e;
	...
	{
		e = EDICT_NUM(i);
		if (!strcmp (pr_strings + e->v.classname, name))
			return e;
	}
	return NULL;
}
...
void Host_GetCoords_f (void)
{
	...
	{
		Q_strncpyz (name, "camfile.cam", sizeof(name));
	}
	else
	{
...
{
	extern void Host_WriteAliases_f (void);
	...
	Cmd_AddCommand ("lastconnect", Host_ConnectToLastServer_f);
...
//	Cmd_AddCommand ("getcoords", Host_GetCoords_f);
	...
	Cmd_AddCommand ("writealiases", Host_WriteAliases_f);//R00k, exports current alias list to a file.
	Cmd_AddCommand ("mcache", Mod_Print);
	Cvar_RegisterVariable (&cl_confirmquit);
	Cvar_RegisterVariable (&show_status_ip);//r00K
	...
	Cmd_AddCommand ("crash", Host_Crash_f);//Baker
}
