...
float CalcFov (float fov_x, float width, float height)
{
		...
		{
                Con_Printf ("Bad fov: %f", fov_x);//R00k was sys_error reset instead!
				Cvar_Set ("fov","90");
		}
...
// decide on the height of the console
	...
	else if (key_dest == key_console)	// added by joe
	{
...
============== 
*/ 
void WritePCX (char *filename, byte *data, int width, int height, int rowbytes, byte *palette)
{
	int	i, j, length;
	pcx_t	*pcx;
	byte	*pack;
...
// write output file 
	...
	if (!COM_WriteFile (filename, pcx, length))
		Con_Printf ("Couldn't save screenshot %s\n", filename);
} 
...
{ 
	...
	else if (Cmd_Argc() == 1)
	{
		...
		for (i=0 ; i<999 ; i++)
		{
			sprintf (name, "qrack%03i.pcx", i);
			if (Sys_FileTime(va("%s/%s", com_gamedir, name)) == -1)
	...
	{
		Con_Printf ("Usage: screenshot [filename]");
		return;
	}
...
// save the pcx file 
	...
	WritePCX (name, vid.buffer, vid.width, vid.height, vid.rowbytes, current_pal);
