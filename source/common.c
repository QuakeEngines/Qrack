...
int	rogue = 0, hipnotic = 0, nehahra = 0, quoth = 0;
...
void MSG_WriteAnglePQ (sizebuf_t *sb, float f)
{
...
void *SZ_GetSpace (sizebuf_t *buf, int length)
{
	...
	if (buf->cursize + length > buf->maxsize)
	{
		if (!buf->allowoverflow)
			Host_Error ("SZ_GetSpace: overflow without allowoverflow set\nbuf->cursize = %i\nlength = %i\nbuf->maxsize = %i", buf->cursize, length, buf->maxsize);
		
		if (length > buf->maxsize)
			Host_Error ("SZ_GetSpace: %i is > buf->maxsize (%i)", length, buf->maxsize);
			
		buf->overflowed = true;
		Con_DPrintf (1,"SZ_GetSpace: overflow of %i bytes.\n", ((buf->cursize + length) - buf->maxsize));
		SZ_Clear (buf); 
	}
...
============
*/
void COM_StripExtension (char *in, char *out) //Fixed function - Eradicator
{
	char *last = NULL;
	while (*in)
	{
...
Use this instead of Sys_FileOpenWrite
=================
*/
int COM_FileOpenWrite (char *path)
{
	FILE	*f;
...
================
*/
//TO-DO: unload/ media in realtime to refresh assests when changing gamedirs.
...
void COM_SetGameDir (char *dir)
{
	...
	extern void	Draw_InitCharset (void);
	...
	if (strcmp(dir, "id1") && strcmp(dir, "qrack"))//dont reload pak files from main folders (?)
	{
		// add the directory to the search path
		search = Q_malloc (sizeof(searchpath_t));
	...
	if (COM_FindFile("config.cfg"))
		Cbuf_AddText ("exec config.cfg\n");//Reload custom cfg for each mod.
