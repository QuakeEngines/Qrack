...
//	eval_rendermode			= FindFieldOffset ("rendermode"); // HalfLife support
	...
	SV_PlayerPhysicsQC 		= ED_FindFunction ("SV_PlayerPhysics");// LordHavoc: allowing QuakeC to override the player movement code	
...
edict_t *ED_Alloc (void)
{
	...
	for (i=svs.maxclients+1 ; i<sv.num_edicts ; i++)
	{
		...
		if (e->free && (e->freetime < 2 || sv.time - e->freetime > 1.0))//R00k doubled the freetime
		{
			ED_ClearEdict (e);
			return e;
		}
...
char *ED_NewString (char *string)
{
	...
	l = strlen(string) + 1;
	new = Hunk_AllocName (l,"NewString");
	new_p = new;
...
qboolean ED_ParseEpair (void *base, ddef_t *key, char *s)
{
	...
	case ev_field:
		if (!(def = ED_FindField(s)))
		{
			...
			if (strcmp(s, "sky") && strncmp(s, "fog", 3) && (strcmp(s, "alpha")))
...
// go through all the dictionary pairs
	...
	// and are immediately discarded by quake
		...
		{
			if (strcmp(keyname, "sky") && strncmp(keyname, "fog", 3) && (strcmp(keyname, "alpha")))
				Con_DPrintf (1,"'%s' is not a field\n", keyname);//set this to DPrint as i get bugreports for this.
			continue;
		}
...
// parse ents
	...
	// parse the opening brace	
		...
		if (com_token[0] != '{')		// "ED_LoadFromFile: found %s when expecting {"
		{			
			Con_DPrintf (1,"ED_LoadFromFile: found %s when expecting {\n",com_token);
			com_token[0] = 0;			//R00k: fuck it! (09/24/2015) 
	...
	Con_DPrintf (1,"%i entities inhibited\n", inhibit);
}
...
Qccx progs need special handling.
===============
*/
void PR_Load_QCCX_Progs (void)
{
	int		i;
...
// flush the non-C variable lookup cache
	...
	if (sv_progs.string == NULL)
		sv_progs.string = "progs.dat";//R00k: dont't crash just provide the standard.

	if (!(progs = (dprograms_t *)COM_LoadHunkFile(sv_progs.string)))
		Sys_Error ("PR_LoadProgs: couldn't load %s", sv_progs.string);

	Con_DPrintf (1,"Programs occupy %iK.\n", com_filesize/1024);
...
// byte swap the header
	...
	if (progs->version != PROG_VERSION)
		Host_Error ("%s has wrong version number (%i should be %i)", sv_progs.string, progs->version, PROG_VERSION);

	if (progs->crc != PROGHEADER_CRC)
		Host_Error ("%s system vars have been modified, progdefs.h is out of date", sv_progs.string);
	...
	for (i=0 ; i<progs->numfunctions; i++)
	{
		...
		if (pr_builtin_remap.value)
		{
			if (pr_functions[i].first_statement < 0)	// builtin function
			{
				...
				{
					Con_DPrintf (1,"Can not assign builtin number #%i to %s - function unknown\n", funcno, funcname);
				}
			}
		}// 2001-09-14 Enhanced BuiltIn Function System (EBFS) by Maddes/Firestorm  end	
	}
	...
	if (pr_builtin_remap.value)
	{
		...
		for ( i=1 ; i < pr_ebfs_numbuiltins; i++)
		{
			if ((!pr_ebfs_builtins[i].funcno) && (pr_ebfs_builtins[i].default_funcno))	// unassigned and has a default number
			{
				...
				{
					Con_DPrintf (1,"Can not assign default builtin number #%i to %s - number is already assigned to %s\n",
...
void PR_Init (void)
{
	...
	// 2001-09-14 Enhanced BuiltIn Function System (EBFS) by Maddes  end
	Cmd_AddCommand ("pr_builtinlist", PR_BuiltInList_f);	
#endif
}
