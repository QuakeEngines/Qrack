...
extern	void Movie_Stop_f (void);//R00k
	...
	if (fdwSupport & ACMDRIVERDETAILS_SUPPORTF_CODEC)
	{
		...
		for (i = 0 ; i < drvDetails.cFormatTags ; i++)
		{
			...
			if (fmtDetails.dwFormatTag == WAVE_FORMAT_MPEGLAYER3)
			{
				...
				Con_DPrintf (1,"MP3-capable ACM codec found: %s\n", drvDetails.szLongName);
				...
				if ((mmr = qacmStreamOpen(&m_mp3_stream, m_mp3_driver, &m_wave_format, &m_mp3_format.wfx, NULL, 0, 0, 0)))
				{
					...
					case MMSYSERR_INVALPARAM:
						Con_DPrintf (1,"ERROR: Invalid parameters passed to acmStreamOpen\n");
						break;

					case ACMERR_NOTPOSSIBLE:
						Con_DPrintf (1,"ERROR: No ACM filter found capable of encoding MP3\n");
						break;

					default:
						Con_DPrintf (1,"ERROR: Couldn't open ACM encoding stream\n");
						break;
					}
					continue;
				}
