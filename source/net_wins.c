...
void WINS_GetLocalAddress(void)
{
	...
	if (COM_CheckParm("-oldnet") || isDedicated) 
	{
		...
		if (WINS_iptype(addr) == 1) 
		{
			...
			Con_DPrintf (1,"WINS_GetLocalAddress: ip address #0 (%s) is invalid, checking next ...\n", my_tcpip_address);
			...
			if (local->h_addr_list[1]) 
			{
				...
				addr = ntohl(myAddr);
				sprintf(my_tcpip_address, "%d.%d.%d.%d", (addr >> 24) & 0xff, (addr >> 16) & 0xff, (addr >> 8) & 0xff, addr & 0xff);
				Con_DPrintf (1,"WINS_GetLocalAddress: ip address #1 (%s) found.  Using.\n", my_tcpip_address);
			} 
			else 
			{
				Con_DPrintf (1,"WINS_GetLocalAddress: ip address #1 does not exist\n");
				Con_DPrintf (1,"WINS_GetLocalAddress: Warning continuing with #0\n");
...
int WINS_Init (void)
{
	...
	if (gethostname (buff, MAXHOSTNAMELEN) == SOCKET_ERROR)
	{
		Con_DPrintf (1,"Winsock TCP/IP Initialization failed.\n");
...
int WINS_OpenSocket (int port)
{
	...
	address.sin_addr.s_addr = INADDR_ANY;//Spike
...
int WINS_GetAddrFromName(char *name, struct qsockaddr *addr)
{
	...
	if (!(hostentry = gethostbyname (name)))
		return -1;
